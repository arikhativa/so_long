...
/*   Updated: 2022/07/17 13:32:50 by yrabby           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	error_print1(int err)
{
	if (ERROR_CHAR_TOP == err)
		...
		error_print_msg("ERROR_CHAR_MID", err);
	else if (ERROR_CHAR_BOTTOM == err)
		error_print_msg("ERROR_CHAR_BOTTOM", err);
	else if (ERROR_SIZE_TOP == err)
		error_print_msg("ERROR_SIZE_TOP", err);
	else if (ERROR_SIZE_MID == err)
		error_print_msg("ERROR_SIZE_MID", err);
	else if (ERROR_SIZE_BOTTOM == err)
		error_print_msg("ERROR_SIZE_BOTTOM", err);
	else if (ERROR_HEIGHT == err)
		error_print_msg("ERROR_HEIGHT", err);
	else if (ERROR_PLAYER == err)
		error_print_msg("ERROR_PLAYER", err);
	else if (ERROR_COLLECT == err)
		error_print_msg("ERROR_COLLECT", err);
	else if (ERROR_EXIT == err)
		error_print_msg("ERROR_EXIT", err);
}
